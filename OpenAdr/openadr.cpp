...
bool Hash_Table::insert(std::string s) {
    size_t i{0};
    auto prob = [this](size_t c, unsigned i) -> size_t {
        return (probing == 1 ? linear_probing(c, i) : double_probing(c, i));
    ...
    while (true) {
        ...
        if (table.at(pos).key() == static_cast<size_t>(key))  //if the key at the position is the same, the element is the same
            return false;
        ++i;
        key = prob(o_key, i);  //o_key is to use the original key in the function and not the new key
        pos = key % table.size();
        if (pos == fpos)
            return false;
    }
